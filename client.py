...
def upload_and_poll(baseurl):
  ...
  try:
    ...
    # loop trying to download
    api = '/results'
    result_url = baseurl + api + '/' + job_id
    processing = False
    res = requests.get(result_url)
    
    while True:
      print("BOOOYAH", res.json())
      ...
      # exit loop when status change
      
        
      if "processing" in status or processing:
    ...
    # display
    if processing and "error" not in status:
      datastr = res.json()
      print("BRUH")
      base64_bytes = datastr.encode()
      print("GG")
